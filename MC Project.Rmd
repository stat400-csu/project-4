...
```{r}
t_test <- t.test(
  filter(results, scenario == "Base (SD=0Ω)")$long_years,
  filter(results, scenario == "Extended (SD=100Ω)")$long_years
)
...
ggplot(results, aes(x = long_years, fill = scenario)) +
  ...
  labs(
    title = "CIED Longevity: Base vs Extended Impedance Variability",
    subtitle = paste("Base: fixed 500Ω | Extended: N(500, 100)Ω | n =", n_patients),
    x = "Longevity (years)",
    y = "Density",
    fill = "Scenario"
  ) +
